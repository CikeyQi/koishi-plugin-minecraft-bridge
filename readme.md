...
| `ciImage`  | 群图片转 CICode 再发到 MC           | `false`               |
| `mask`     | MC → 群消息过滤正则                 | `''`                  |
...
在其他 Koishi 插件中可直接注入 `mcBridge` 服务。
...
export function apply(ctx: Context) {
  ctx.on('mc-bridge/event', async (evt) => {
    if (evt.sub_type !== 'player_join') return
    if (!evt.server_name) return
    const name = evt.player.nickname
    if (!name) return
    try {
      await ctx.mcBridge.request(
        'broadcast',
        { message: `欢迎 ${name} 加入游戏` },
        { selfName: evt.server_name },
      )
    } catch (err) {
      const { code, message } = err as { code?: string, message?: string }
      ctx.logger('demo').warn(`欢迎语发送失败 code=${code || '-'} msg=${message || 'unknown'}`)
    }
  })
}
```
...
* 调用入口：`ctx.mcBridge.request(api, data, options?)`
* 单服务器在线时可省略 `options.selfName`；多服务器在线时必须传 `selfName`
